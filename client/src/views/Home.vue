<template>
    ...
    <v-card-actions>
      <v-row>
        ...
        <v-col v-if="logged">
          ...
          <TodoCreate v-model="createTodo.show" @close_todo_create="createTodo.show = false" />
        </v-col>
      </v-row>
    </v-card-actions>
  ...
  <TodoUpdate v-model="todoInfo.show" :todo="todoInfo.currentTodo" @close_todo_info="todoInfo.show = false" />
</template>
...
export default {
  name: 'HomeView',
  components: {TodoCreate, TodoUpdate},
  data() {
    return {
      todos: [],
      currentPage: 1,
